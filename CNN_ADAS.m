...
for CV_idx = 1 : KFold_Num
    for X_ADAS_idx = 1 : size(X_ADAS,2)
        X_train_CV{X_ADAS_idx} = [];X_test_CV{X_ADAS_idx} = [];
        for idx = 1 : size(X_ADAS{4},1)
            if CV_Indice(idx) ~= CV_idx
                X_train_CV{X_ADAS_idx} = [X_train_CV{X_ADAS_idx};X_ADAS{X_ADAS_idx}(idx,:)];
            else
                X_test_CV{X_ADAS_idx}  = [X_test_CV{X_ADAS_idx};X_ADAS{X_ADAS_idx}(idx,:)];
 ...
 for Mdl_idx = 1 : size(X_ADAS,2)-1
        X_train = X_train_CV{Mdl_idx}; Y_train = X_train_CV{size(X_ADAS,2)};
        X_test  = X_test_CV{Mdl_idx};  Y_test  = X_test_CV{size(X_ADAS,2)};
 ...
 RMSE_CNN_ADAS_avg = mean(RMSE_CNN,3);
 Std_CNN_ADAS      = std(RMSE_CNN,0,3);
 save('CNN_VBM_ADAS.mat','RMSE_CNN_ADAS_avg','Std_CNN_ADAS');
