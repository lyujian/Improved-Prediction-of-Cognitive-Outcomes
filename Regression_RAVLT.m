...
if   sum(isnan(ADNI_VBM_MIL{idx}.RAVLT_BL))==0    && sum(isempty(ADNI_VBM_MIL{idx}.RAVLT_BL))==0 ...
    ...
    for p_para_idx = 1 : size(W_para,2)
       X_RAVLT{X_idx} = [X_RAVLT{X_idx};ADNI_VBM_MIL{idx}.Base*W_para{Dim_r_idx,p_para_idx,idx}];
       X_idx = X_idx + 1;
    end
    end
    
    for Dim_r_idx = 1 : size(W_para,1)
       X_RAVLT{X_idx} = [X_RAVLT{X_idx};ADNI_VBM_MIL{idx}.Base*W_PCA_KNN{Dim_r_idx,idx}'];
       X_idx = X_idx + 1;        
    end    
    X_RAVLT{X_idx} = [X_RAVLT{X_idx};ADNI_VBM_MIL{idx}.RAVLT_BL];
end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
KFold_Num = 5;
CV_Indice = crossvalind('kfold',size(X_RAVLT{X_idx},1),KFold_Num );
for CV_idx = 1 : KFold_Num
    for X_RAVLT_idx = 1 : size(X_RAVLT,2)
        X_train_CV{X_RAVLT_idx} = [];X_test_CV{X_RAVLT_idx} = [];
        for idx = 1 : size(X_RAVLT{4},1)
            if CV_Indice(idx) ~= CV_idx
                X_train_CV{X_RAVLT_idx} = [X_train_CV{X_RAVLT_idx};X_RAVLT{X_RAVLT_idx}(idx,:)];
            else
                X_test_CV{X_RAVLT_idx}  = [X_test_CV{X_RAVLT_idx};X_RAVLT{X_RAVLT_idx}(idx,:)];
                
            end
        end
    end
    
   for Mdl_idx = 1 : size(X_RAVLT,2)-1
        X_train = X_train_CV{Mdl_idx}; Y_train = X_train_CV{size(X_RAVLT,2)};
        X_test  = X_test_CV{Mdl_idx};  Y_test  = X_test_CV{size(X_RAVLT,2)};
