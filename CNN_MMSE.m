...
for CV_idx = 1 : KFold_Num
    for X_MMSE_idx = 1 : size(X_MMSE,2)
        X_train_CV{X_MMSE_idx} = [];X_test_CV{X_MMSE_idx} = [];
        for idx = 1 : size(X_MMSE{4},1)
            if CV_Indice(idx) ~= CV_idx
                X_train_CV{X_MMSE_idx} = [X_train_CV{X_MMSE_idx};X_MMSE{X_MMSE_idx}(idx,:)];
            else
                X_test_CV{X_MMSE_idx}  = [X_test_CV{X_MMSE_idx};X_MMSE{X_MMSE_idx}(idx,:)];
 ...
 for Mdl_idx = 1 : size(X_MMSE,2)-1
        X_train = X_train_CV{Mdl_idx}; Y_train = X_train_CV{size(X_MMSE,2)};
        X_test  = X_test_CV{Mdl_idx};  Y_test  = X_test_CV{size(X_MMSE,2)};
 ...
 RMSE_CNN_MMSE_avg = mean(RMSE_CNN,3);
 Std_CNN_MMSE      = std(RMSE_CNN,0,3);
 save('CNN_VBM_MMSE.mat','RMSE_CNN_MMSE_avg','Std_CNN_MMSE');
